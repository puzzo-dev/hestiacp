...
| 19 | `mkdir -p` and `chown` on the app-root base followed a symlink at `.ivarse`. | High. Measured: it created `/etc/apps`, and would have chowned wherever the link pointed. Not reachable by an unprivileged user today, since `/home/<user>` is root-owned, but it is the component every other guarantee rests on. | `F2c` review — the parent chain is built one component at a time and any component that is already a symlink is refused |
